...
def draw_circle_flower(dwg, cx, cy, arms=8, r2=60):
    ...
    print(f'r1 = {r1} r2 = {r2} step = {step}')

    for i in range(arms):
        angle_deg = (360 / arms) * i
        ...
        for r in range(r1, r2, step):
            # Center of circle is offset by exactly r in the direction
            x = cx + r * dx
            y = cy + r * dy
...
def main():
    # Canvas
    W, H = 250, 350
    dwg = svgwrite.Drawing("player_card_back_design_3.svg", size=(W, H))
    ...
    print(f"rect_w={rect_w} rect_h={rect_h}" )
    pat_size_x = rect_w / 10
    pat_size_y = rect_h / 15
    hatch = dwg.pattern(
        id="dotted_hatch",
    ...
    line_style_1 = {"stroke": CARD_COLOR, "stroke_width": 3}
    ...
    hatch.add(dwg.line(start=((pat_size_x/2), 0), end=(pat_size_x, (pat_size_y/2)), **line_style_2))   # top -> right
    hatch.add(dwg.line(start=(pat_size_x, (pat_size_y/2)), end=((pat_size_x/2), pat_size_y), **line_style_2))   # right -> bottom
    hatch.add(dwg.line(start=((pat_size_x/2), pat_size_y), end=(0, (pat_size_y/2)), **line_style_2))   # bottom -> left
    hatch.add(dwg.line(start=(0, (pat_size_y/2)), end=((pat_size_x/2), 0), **line_style_2))   # left -> top
    ...
    draw_circle_flower(dwg, cx, cy, 12, 30)
    ...
    # Save
    dwg.save()
    print("player_card_back_design_3.svg")
